...
void *hkGetDataFor(void *thisptr, IHyprWindowDecoration *pDecoration,
                   PHLWINDOW pWindow) {
  if (DecorationsWrapper *wrapper =
          dynamic_cast<DecorationsWrapper *>(pDecoration)) {
    // Debug::log(LOG, "IGNORE: Decoration {}", (void*)pDecoration);
    pDecoration = wrapper->get();
  }

  return ((decltype(&hkGetDataFor))g_getDataForHook->m_original)(
      thisptr, pDecoration, pWindow);
}
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
  ...
  HyprlandAPI::addConfigValue(PHANDLE, "plugin:darkwindow:ignore_decorations",
  ...
  g_Callbacks.push_back(HyprlandAPI::registerCallbackDynamic(
      PHANDLE, "render", [&](void *self, SCallbackInfo &, std::any data) {
        std::lock_guard<std::mutex> lock(g_InverterMutex);
  ...
  g_Callbacks.push_back(HyprlandAPI::registerCallbackDynamic(
      PHANDLE, "closeWindow", [&](void *self, SCallbackInfo &, std::any data) {
        std::lock_guard<std::mutex> lock(g_InverterMutex);
  ...
  static const auto METHOD = ([&] {
    ...
    auto found =
        std::find_if(all.begin(), all.end(), [](const SFunctionMatch &line) {
          return line.demangled.starts_with(
              "CDecorationPositioner::getDataFor");
        });
    if (found != all.end())
  ...
  if (METHOD) {
    g_getDataForHook = HyprlandAPI::createFunctionHook(handle, METHOD->address,
                                                       (void *)&hkGetDataFor);
    g_getDataForHook->hook();
  } else {
    Debug::log(WARN,
               "[DarkWindow] Failed to hook CDecorationPositioner::getDataFor, "
...
Hyprlang::CParseResult onInvertKeyword(const char *COMMAND, const char *VALUE) {
  if (!g_DidNotify) {
    ...
    HyprlandAPI::addNotification(
        PHANDLE,
        "[Hypr-DarkWindow] The darkwindow_invert keyword was removed in favor "
        "of windowrulev2s please check the GitHub for more info.",
        {0xFF'00'00'00}, 10000);
  }
  return {};
}
...
// TODO remove deprecated
static void addDeprecatedEventListeners() {
  HyprlandAPI::addConfigKeyword(PHANDLE, "chromakey_enable", onInvertKeyword,
                                {.allowFlags = false});
}
