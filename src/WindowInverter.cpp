...
void WindowInverter::OnRenderWindowPre() {
  ...
  bool shouldInvert =
      ...
      (std::find(m_ManuallyInvertedWindows.begin(),
...
void WindowInverter::OnRenderWindowPost() {
  if (m_ShadersSwapped) {
    if (m_DecorationsWrapped) {
      for (auto &decoration : g_pHyprOpenGL->m_renderData.currentWindow.lock()
                                  ->m_windowDecorations) {
        // Debug::log(LOG, "REMOVE: Window {:p}, Decoration {:p}",
        // (void*)g_pHyprOpenGL->m_pCurrentWindow.get(),
        // (void*)decoration.get());
        // if (DecorationsWrapper *wrapper =
        //        dynamic_cast<DecorationsWrapper *>(decoration.get()))
...
void WindowInverter::OnWindowClose(PHLWINDOW window) {
  static const auto remove = [](std::vector<PHLWINDOW> &windows,
                                PHLWINDOW &window) {
...
void WindowInverter::InvertIfMatches(PHLWINDOW window) {
  // for some reason, some events (currently `activeWindow`) sometimes pass a
  // null pointer
  if (!window)
    return;

  std::vector<SP<CWindowRule>> rules =
      g_pConfigManager->getMatchingRules(window);
  bool shouldInvert =
      std::any_of(rules.begin(), rules.end(), [](const SP<CWindowRule> &rule) {
...
void WindowInverter::ToggleInvert(PHLWINDOW window) {
  ...
  auto windowIt = std::find(m_ManuallyInvertedWindows.begin(),
                            m_ManuallyInvertedWindows.end(), window);
...
void WindowInverter::SoftToggle(bool invert) {
  if (m_ShadersSwapped) {
    ...
    toggleInvert(g_pHyprOpenGL->m_shaders->m_shEXT.program,
                 m_Shaders.EXT_Invert);
    toggleInvert(g_pHyprOpenGL->m_shaders->m_shRGBA.program,
                 m_Shaders.RGBA_Invert);
    toggleInvert(g_pHyprOpenGL->m_shaders->m_shRGBX.program,
                 m_Shaders.RGBX_Invert);
  }
}
...
void WindowInverter::Reload() {
  ...
  if (m_IgnoreDecorations) {
    Hyprlang::CConfigValue *config = HyprlandAPI::getConfigValue(
        m_PluginHandle, "plugin:darkwindow:ignore_decorations");
